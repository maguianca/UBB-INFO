...
$stmt = $conn->prepare("
    SELECT * FROM trenuri
    WHERE loc_plecare = ? AND loc_sosire = ?
");
$stmt->bind_param("ss", $plecare, $sosire);
$stmt->execute();
$rezultat = $stmt->get_result();

echo "<h2>Curse directe din $plecare_safe în $sosire_safe:</h2>";
...
if ($legatura) {
    ...
    while ($row = $rezultat2->fetch_assoc()) {
        echo "<p>
            Prima etapa: {$row['plecare1']} → {$row['legatura']} (plecare la {$row['ora_plecare1']}, sosire la {$row['ora_sosire1']})<br>
            A doua etapa: {$row['legatura2']} → {$row['sosire2']} (plecare la {$row['ora_plecare2']}, sosire la {$row['ora_sosire2']})
        </p>";
    }
}
