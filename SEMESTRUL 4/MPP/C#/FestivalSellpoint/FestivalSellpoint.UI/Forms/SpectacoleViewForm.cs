...
{
    public partial class SpectacoleViewForm : AppForm, IObserver
    {
        Angajat Angajat;
        ...
        void Login()
        {
            var loginForm = new AngajatLoginForm(AppService, this);
        ...
        public void UpdateSpectacol(Spectacol s)
        {
            BeginInvoke(new Action(() =>
            {
                foreach (var spec in Spectacole)
                    if (spec.Id == s.Id)
                    {
                        spec.NrLocuriDisponibile = s.NrLocuriDisponibile;
                        spec.NrLocuriVandute = s.NrLocuriVandute;
                        break;
                    }

                foreach (var spec in FilteredSpectacole)
                    if (spec.Spectacol.Id == s.Id)
                    {
                        spec.Spectacol.NrLocuriDisponibile = s.NrLocuriDisponibile;
                        spec.Spectacol.NrLocuriVandute = s.NrLocuriVandute;
                        break;
                    }
        ...
        private void SpectacoleView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
