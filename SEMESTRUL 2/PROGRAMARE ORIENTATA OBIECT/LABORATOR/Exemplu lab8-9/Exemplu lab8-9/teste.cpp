...
void test_fisier() {
    OfertaRepoFile repo{
            "C:\\Programare orientata obiect\\Exemplu lab8-9\\Exemplu lab8-9\\test.txt" };
    Validator valid;
    CosOferte cos;
    AgentieService service{ repo, valid, cos };
    assert(service.getAll().size() == 2);
...
void test_cos() {
    ...
    // test export
    string fisier_csv = "C:\\Programare orientata obiect\\Exemplu lab8-9\\Exemplu lab8-9\\test.csv";
    string fisier_html = "C:\\Programare orientata obiect\\Exemplu lab8-9\\Exemplu lab8-9\\test.html";
    string fisier_alt = "C:\\Programare orientata obiect\\Exemplu lab8 - 9\\Exemplu lab8 - 9\\test-cos.txt";
...
void test_repo_lab()
{
    ...
    Oferta oferta1("Paris", "Europa", "City", 1000);
    repo.adauga(oferta1);

    auto toate = repo.getAll();
    assert(toate.size() == 1);
    assert(toate[0].getDenumire() == "Paris");
    repo.stergere(oferta1);
    auto toate1 = repo.getAll();
    assert(toate1.size() == 0);
    try {
        repo.cauta("pae","pib"); assert(false);
    }
    catch (Exception& ex) {
        //assert(true);
        assert(ex.getMessage() == "Oferta nu exista.");
    }
    Oferta oferta2("Paris", "Europa", "City", 1000);
    Oferta oferta3("Paris1", "Europa1", "City1", 1000);
    Oferta oferta4("Paris2", "Europa2", "City2", 1000);
    repo.adauga(oferta2);
    repo.adauga(oferta3);
    repo.adauga(oferta4);
    auto x = repo.cauta("Paris1", "Europa1");
    repo.stergere(oferta2);
    try {
        repo.adauga(oferta3); assert(false);
    }
    catch (Exception& ex) {
        //assert(true);
        assert(ex.getMessage() == "Oferta exista deja in lista.");
    }
    repo.stergere(oferta4);
    repo.setProbabilitate(0);
    try {
        repo.adauga(oferta4); assert(false);
    }
    catch (Exception& ex) {
        //assert(true);
        assert(ex.getMessage() == "Nu se poate executa aceasta functie!!!");
    }
}
void test_all() {
    // Teste REPOSITORY
