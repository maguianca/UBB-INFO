...
void GUI::paintEvent(QPaintEvent* ev) {
	...
	for (int i=0;i<=10;i++)
	{
		...
		p.drawRect(300 + i * 10, p.device()->height()-10, 10, -(x + 1) * 20);
	}
}
void GUI::conectare()
{
	...
	QObject::connect(lista->selectionModel(), &QItemSelectionModel::selectionChanged, this, [&]() {
		if (lista->selectionModel()->selectedIndexes().isEmpty())
			...
			auto row = lista->selectionModel()->selectedIndexes()[0].row();
			string text=lista->model()->data(lista->model()->index(row, 0), Qt::DisplayRole).toString().toStdString();
			int id, rank,item_no=0;
			string titlu, artist,current_item;
