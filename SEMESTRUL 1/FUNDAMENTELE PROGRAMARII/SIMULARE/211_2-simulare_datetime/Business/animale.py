...
class BusinessAnimale:
    ...
    def date(self,data1,data2):
        params1 = data1.split("/")
        params2 = data2.split("/")
        data1=date(int(params1[0]),int(params1[1]),int(params1[2]))
        data2 = date(int(params2[0]), int(params2[1]), int(params2[2]))
        if data1<data2:
            return 1
        elif data1>data2:
            return 2
    ...
    def statistica(self):
        ...
        if stat=={}:
            ...
            return stat
def test_comparare():
    repo=FileRepoAnimale("C:\\211_2-simulare_datetime\\Teste\\t.txt")
    service=BusinessAnimale(repo)
    assert(service.date("2023/1/1","2024/1/1")==1)
    assert (service.date( "2024/1/1","2023/1/1",) == 2)
    assert(service.date("2020/2/2","2020/2/2")==0)
test_comparare()
def test_filtrare():
    repo = FileRepoAnimale("C:\\211_2-simulare_datetime\\Teste\\t.txt")
    service = BusinessAnimale(repo)
