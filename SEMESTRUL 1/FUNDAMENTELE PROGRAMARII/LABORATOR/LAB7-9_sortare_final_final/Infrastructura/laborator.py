...
class FileRepoLaborator(LaboratorRepository):
    ...
    #recursiv
    def cauta_numar(self, numar, index=0):
        """
        cauta recursiv un laborator din lista dupa numar
        :param numar: numar laborator
        :param index: indexul listei de laboratoare
        :return: laborator, altfel raise RepoError
        """
        self.__read_all_laboratoare_from_file()

        if index < len(self._toatelaboratoarele):
            lab_curent = list(self._toatelaboratoarele.values())[index]

            if lab_curent.get_numar() == numar:
                return lab_curent

            return self.cauta_numar(numar, index + 1)
        else:
            raise RepoError("nu exista nr!\n")
