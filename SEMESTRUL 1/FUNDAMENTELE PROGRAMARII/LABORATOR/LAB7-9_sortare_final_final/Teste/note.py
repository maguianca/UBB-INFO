...
class TesteN:
    ...
    def __test_modificare_nota(self):
        student = Student(1, 'Anca', 12)
        laborator = Laborator('1_5', 'app', '12/12/2025')
    ...
    def __test_insertion_sort(self):
        student1 = Student(1, 'Anca', 12)
        student2 = Student(2, 'Elena', 12)
        student3 = Student(3, 'Amalia', 10)
        student4 = Student(4, 'Eugenia', 11)
        lista_nesortata=[student1,student2,student3,student4]
        repo_studenti = StudentRepository()
        repo_laborator = LaboratorRepository()
        repo_note = NoteRepository()
        validator_note = ValidatorNote()
        business_note = NoteBusiness(repo_laborator, repo_studenti, repo_note, validator_note)
        list=business_note.insertion_sort_key(lista_nesortata,key=lambda x:x.get_nume())
        assert list==[student3,student1,student2,student4]
        list2=business_note.insertion_sort_key(lista_nesortata,key=lambda x:(x.get_grup(),x.get_nume()))
        assert list2==[student3,student4,student1,student2]
    ...
    def __test_comb_sort(self):
        ...
        validator_note = ValidatorNote()
        business_note = NoteBusiness(repo_laborator, repo_studenti, repo_note, validator_note)
        list = business_note.comb_sort_key(lista_nesortata, key=lambda x: x.get_nume())
        assert list == [student3, student1, student2, student4]
        list2 = business_note.comb_sort_key(lista_nesortata, key=lambda x: (x.get_grup(), x.get_nume()))
        assert list2 == [student3, student4, student1, student2]
