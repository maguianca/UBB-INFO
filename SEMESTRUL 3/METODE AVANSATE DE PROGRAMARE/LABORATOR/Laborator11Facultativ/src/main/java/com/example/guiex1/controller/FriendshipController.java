...
public class FriendshipController implements Observer<UtilizatorEntityChangeEvent> {
        ...
        FriendsList.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2) {
                ...
                if (selectedFriend != null) {
                    openUserProfile(selectedFriend);
                }
            }
        });
    }
    private void openUserProfile(Utilizator selectedFriend) {
        String username = selectedFriend.getUsername();
        int prieteni= service.getFriends(selectedFriend.getId()).size();
        String profileImageUrl = "/com/example/guiex1/images/mpp_pic.png";
        String nume=selectedFriend.getFirstName()+" "+selectedFriend.getLastName();
        int comuni=service.getNumberMutualFriends(selectedFriend.getId(), utilizator.getId());
        UserProfile userProfile = new UserProfile(username,nume,prieteni,comuni, selectedFriend.getUrl(), selectedFriend.getDescriere());
        userProfile.show();
    }
    ...
    public void handleprofile(ActionEvent actionEvent) {
        ...
        int prieteni= service.getFriends(utilizator.getId()).size();
        String profileImageUrl = "/com/example/guiex1/images/mpp_pic.png";
        String nume=utilizator.getFirstName()+" "+utilizator.getLastName();
        int comuni=0;
        UserProfile userProfile = new UserProfile(username,nume,prieteni,comuni, utilizator.getUrl(),utilizator.getDescriere());
        userProfile.show();
    }
}
